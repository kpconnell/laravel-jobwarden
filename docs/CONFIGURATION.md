...
| `JOBWARDEN_PREFORK_RECYCLE_AFTER` | `50000` | `prefork` only — forks after which the master wants a fresh copy-on-write baseline. `0` disables. Crossing it does **not** stop claiming: a drain idles every slot until the longest in-flight job ends, so the master keeps working and recycles at the first moment it has nothing in flight. |
| `JOBWARDEN_PREFORK_RECYCLE_GRACE` | `0` | How long a wanted recycle waits for that idle moment before force-draining anyway. `0` (default) = never force — recycle only at a truly idle tick, so a busy lane is never stalled; a lane that never idles then never rebaselines (deferral logged every minute). A positive grace opts into the forced drain, which claims nothing until the last in-flight fork exits — any job longer than the grace wedges its lane for that job's remaining runtime, at every recycle. A recycle drain ignores `JOBWARDEN_DRAIN_TIMEOUT` — housekeeping must never abandon healthy children to the reaper's `SIGKILL`. |
| `jobwarden.supervisor.prefork_forget` *(config-only — publish to override; a list doesn't belong in an env string)* | `ForkExecutor::DEFAULT_FORGET` = `redis`, `cache`, `cache.store`, `cache.psr6`, `Illuminate\Cache\RateLimiter::class`, `queue`, `queue.connection`, `filesystem`, `mail.manager`, `log` | `prefork` only — container services a fork must not inherit. Right after the fork the child drops each listed binding (alias-normalized; only shared bindings with a concrete that the master actually resolved — `$app->instance()` registrations are skipped, there is nothing to rebuild them from) and clears **every** facade's static cache, so first use inside the handler (facade or container) rebuilds a fresh instance with its own sockets instead of sharing the master's. Class-name keys and container aliases are fine. A published config that predates this key falls back to the shipped list. Forgetting a key does **not** fix references captured before the fork (a manager constructor-injected into an app singleton stays the master's) — reset those, and anything the container can't name, in a `JobWarden\Events\PreforkChildStarting` listener. Never list `events`. Persistent connections (`PDO::ATTR_PERSISTENT`, phpredis `persistent`) are incompatible with prefork; a PDO-persistent `jobwarden` connection makes the supervisor fall back to `child` mode with a warning. See [HOSTING → Fork safety](HOSTING.md#fork-safety-what-jobwarden-resets-and-what-your-app-must). |
