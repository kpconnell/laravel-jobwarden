...
| `jobwarden.supervisor.prefork_forget` *(config-only — publish to override; a list doesn't belong in an env string)* | `redis`, `cache`, `cache.store`, `cache.psr6`, `Illuminate\Cache\RateLimiter::class`, `queue`, `mail.manager`, `log` | `prefork` only — container services a fork must not inherit. Right after the fork the child drops each binding — the container instance **and the facade's static cache**, on the alias-normalized key, and only for shared bindings the master actually resolved — so first use inside the handler (facade or container) rebuilds a fresh instance with its own sockets instead of sharing the master's. Class-name keys and container aliases are fine. Forgetting a key does **not** fix references captured before the fork (a manager constructor-injected into an app singleton stays the master's) — reset those, and anything the container can't name, in a `JobWarden\Events\PreforkChildStarting` listener. Never list `events`. See [HOSTING → Fork safety](HOSTING.md#fork-safety-what-jobwarden-resets-and-what-your-app-must). |
