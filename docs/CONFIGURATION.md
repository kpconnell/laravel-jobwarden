...
| `JOBWARDEN_PREFORK_RECYCLE_GRACE` | `3600` | How long a wanted recycle waits for that idle moment before draining anyway (the stall, accepted as a rare fallback so a permanently busy host still rebaselines). `0` = never force. A recycle drain ignores `JOBWARDEN_DRAIN_TIMEOUT` — housekeeping must never abandon healthy children to the reaper's `SIGKILL`. |
